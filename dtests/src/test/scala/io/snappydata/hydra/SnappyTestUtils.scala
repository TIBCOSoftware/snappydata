 ...
 */
  ...
  def assertQueryFullResultSet(snc: SnappyContext, snDF : DataFrame,
                               spDF : DataFrame, queryNum: String, tableType: String,
                               pw: PrintWriter, sqlContext: SQLContext, isJoin : Boolean ): Any = {
  ...
  def assertQueryFullResultSetHiveThriftServer(snc: SnappyContext, sqlString: String,
                                               queryNum: String, tableType: String, pw: PrintWriter,
                                               sqlContext: SQLContext): Any = {
    var snappyDF: DataFrame = null
    //    if(!usePlanCaching) {
    //      snappyDF = snc.sqlUncached(sqlString)
    //    } else {
