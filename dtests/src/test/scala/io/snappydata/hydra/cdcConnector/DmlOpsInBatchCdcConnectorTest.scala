...
class DmlOpsInBatchCdcConnectorTest extends SnappyHydraTestRunner {
  ...
  def truncateSqlTable(): Unit = {
    ...
    val conn = DriverManager.getConnection(
      "jdbc:sqlserver://sqlent.westus.cloudapp.azure.com:1433;DatabaseName=testdatabase20g",
  ...
  def before(): Unit = {
    ...
    logInfo("Started snappy cluster")
    ...
    logInfo("Table created and jar deployed")
    logInfo("appJar " + appJar)
  ...
  def after(): Unit = {
    ...
    logInfo("Stopped snappy cluster")
  }
  ...
  def performValidation(sqlResultSet: ResultSet, snappyResultSet: ResultSet): Unit = {
    ...
    logInfo("Row cnt of snappy and sql table is = "
        + snappyRowsCnt + " " + sqlRowsCnt)
    logInfo("Column cnt of snappy and sql table is = "
