...
public class SnappyPreparedStmtTest extends SnappyTest {
  ...
  public void executePreparedStmtsConc(Connection conn, String queryFilePathPS, String queryFilePath){
    try{
    for (int q = 0; q <= queryArr.length - 1; q++) {
      Random rnd = new Random();
      String queryName = queryArr[rnd.nextInt(queryArr.length - 1)];
  ...
  public void executePreparedStmts() {
    ...
    try {
      ...
      executePreparedStmtsConc(conn,queryFilePathPS,queryFilePath);
    }
  catch (SQLException se) {
      throw new TestException("QUERY FAILED. Exception is : \n" + se.getSQLState() + " : " + se.getMessage());
    } finally {
