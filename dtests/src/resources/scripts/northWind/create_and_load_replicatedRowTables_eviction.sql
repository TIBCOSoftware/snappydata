...
CREATE TABLE categories USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT CategoryID, CategoryName,
...
create table shippers USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT ShipperID, CompanyName, Phone FROM staging_shippers);
...
CREATE TABLE employees USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT EmployeeID, LastName,  FirstName, Title,
...
CREATE TABLE customers USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT CustomerID, CompanyName, ContactName, ContactTitle,
...
CREATE TABLE orders USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT OrderID, CustomerID, EmployeeID, OrderDate,
...
CREATE TABLE order_details USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT OrderID, ProductID, UnitPrice,
...
CREATE TABLE products USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT ProductID, ProductName, SupplierID, CategoryID,
...
CREATE TABLE suppliers USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT SupplierID, CompanyName, ContactName,
...
CREATE TABLE territories USING row OPTIONS(EVICTION_BY ':evictionByOption') AS (SELECT TerritoryID, TerritoryDescription, RegionID
