...
CREATE TABLE orders USING column OPTIONS(partition_by 'OrderId', buckets '13', PERSISTENT ':persistenceMode') AS (SELECT OrderID, CustomerID, EmployeeID, OrderDate,
...
CREATE TABLE order_details USING column OPTIONS(partition_by 'OrderId', buckets '13', COLOCATE_WITH 'orders', PERSISTENT ':persistenceMode') AS (SELECT OrderID, ProductID, UnitPrice,
...
CREATE TABLE products USING column OPTIONS(partition_by 'ProductID,SupplierID', buckets '17', PERSISTENT ':persistenceMode') AS (SELECT ProductID, ProductName, SupplierID, CategoryID,
