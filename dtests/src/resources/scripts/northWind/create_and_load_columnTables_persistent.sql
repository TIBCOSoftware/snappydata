...
CREATE TABLE employees USING column OPTIONS(PERSISTENT ':persistenceMode') AS (SELECT EmployeeID, LastName,  FirstName, Title,
...
CREATE TABLE orders USING column OPTIONS(partition_by 'OrderId', buckets '13', PERSISTENT ':persistenceMode', redundancy '1') AS (SELECT OrderID, CustomerID, EmployeeID, OrderDate,
...
CREATE TABLE order_details USING column OPTIONS(partition_by 'OrderId', buckets '13', COLOCATE_WITH 'orders', PERSISTENT ':persistenceMode', redundancy '1') AS (SELECT OrderID, ProductID, UnitPrice,
...
CREATE TABLE products USING column OPTIONS(partition_by 'ProductID,SupplierID', buckets '17', PERSISTENT ':persistenceMode', redundancy '1') AS (SELECT ProductID, ProductName, SupplierID, CategoryID,
