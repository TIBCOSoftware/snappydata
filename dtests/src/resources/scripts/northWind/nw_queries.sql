...
SELECT COUNT(DISTINCT e.EmployeeID) AS numEmployees,  COUNT(DISTINCT c.CustomerID) AS numCompanies,  e.City as employeeCity, c.City as customerCity  FROM Employees e JOIN Customers c ON  (e.City = c.City)  where e.EmployeeID > 3   GROUP BY e.City, c.City   ORDER BY numEmployees DESC ;
