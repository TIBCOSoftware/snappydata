...
class StreamSummaryAggregation[T](val capacity: Int, val intervalSize: Long, val epoch0: Long,
          ...
          timeEpoch.timestampToInterval(epoch) match {
            case Some(interval) => {
              if (interval <= maxIntervals)
                streamAggregates(interval -1).offer(item.key, item.frequency.toInt)
              // else IGNORE
  ...
  def queryIntervals (start:Int, end : Int, k: Int) = {
    ...
    topkFromAllIntervals groupBy(_.getItem) map(x =>
      (x._1, new Approximate((x._2.foldLeft(0L)(_ + _.getError)) , (x._2.foldLeft(0L)(_ + _.getCount)), 0 , 0))
      )
  }

  def getTopKBetweenTime(epochFrom: Long, epochTo: Long, k : Int): Option[Array[(T, Approximate)]] =
    this.rwlock.executeInReadLock({
      val (end, start) = this.timeEpoch.convertEpochToIntervals(epochFrom, epochTo) match {
        case Some(x) => x
