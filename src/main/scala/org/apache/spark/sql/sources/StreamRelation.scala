...
private[sql] class StreamSource extends SchemaRelationProvider {
  ...
  def createRelation(sqlContext: SQLContext,
    ...
    val classinstance = loadFormatClass(OptsUtil.getOption(OptsUtil.FORMAT, options)).newInstance()
    val formatFunction =  if (classinstance.isInstanceOf[UserDefinedInterpreter[String]])
      classinstance.asInstanceOf[UserDefinedInterpreter[String]].formatter()_
    else throw new IllegalStateException("Incorrect format function")

    // Create a dstream here based on the parameters passed as part of create stream
    val dstream = StreamingCtxtHolder.streamingContext.socketTextStream(host, port)


    options.get(OptsUtil.WINDOWDURATION) match {
      case Some(wd) => options.get(OptsUtil.SLIDEDURATION) match {
        case Some(sd) => dstream.window(Duration(wd.toInt), Duration(sd.toInt))
        case None => dstream.window(Duration(wd.toInt))
      }
      case None => "no-op"
    }
...
object OptsUtil {
  ...
  def getOption(optionName : String, options:Map[String, String]): String =  options.getOrElse(optionName,
    throw new IllegalStateException(s"$optionName name not defined"))
...
private[sql] case class CreateStreamTableCmd(
                                              streamName: String,
  ...
  def run(sqlContext: SQLContext): Seq[Row] = {
    ...
    sqlContext.asInstanceOf[SnappyContext].catalog.streamTables.put(streamName, plan)
    Seq.empty
  }
}

private[sql] case class StreamingCtxtActionsCmd(action: Int,
                                                batchInterval: Option[Int]) extends RunnableCommand {
    ...
    action match {
      ...
      case 1 => {
        ...
        catalog.streamTables.foreach(streamEntry => {
          ...
          val sampleTables = catalog.streamToSampleTblMap.getOrElse(streamEntry._1, Nil)

          val sr = catalog.getStreamTableRelation(streamEntry._1).asInstanceOf[StreamRelation[_]]
          ...
          snappyCtxt.saveStream[String](sr.dStream.asInstanceOf[DStream[String]],
            sampleTables, sr.formatter.asInstanceOf[(RDD[String], StructType) => RDD[Row]], sr.schema)
      ...
      case 2 => {
        StreamingCtxtHolder.streamingContext.stop()
      }
    }
    Seq.empty[Row]
  }
...
private[sql] case class CreateSampledTableCmd(
  ...
  def run(sqlContext: SQLContext): Seq[Row] = {
    ...
    snappyCtxt.registerSampleTable(sampledTableName, sr.schema, options - OptsUtil.BASETABLE)
