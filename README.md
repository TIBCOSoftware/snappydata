...
* [Getting started](#getting-started)
  ...
  * [Start the SnappyData Cluster](#start-the-snappydata-cluster)
    * [Action - Start the SnappyData cluster](#action---start-the-snappydata-cluster)
  * [Spark and SnappyData](#spark-and-snappydata)
  * [Interacting with SnappyData](#interacting-with-snappydata)
  * [Column and Row tables](#column-and-row-tables)
    * [Action - Create column table, row table and load data](#action---create-column-table-row-table-and-load-data)
  * [OLAP and OLTP queries](#olap-and-oltp-queries)
    * [Action - Run OLAP and OLTP queries](#action---run-olap-and-oltp-queries)
  * [Approximate query processing (AQP)](#approximate-query-processing-aqp)
    * [Action - Create, Load and Query Sample Table](#action---create-load-and-query-sample-table)
...
#### Action - Start the SnappyData cluster 
...
To interact with SnappyData, we provide interfaces for developers familiar with Spark programming as well as SQL. For SQL, the SnappyData SQL Shell (_snappy-shell_) provides a simple way to inspect the catalog,  run admin operations,  manage the schema and run interactive queries. You can also use your favorite SQL tool like SquirrelSQL or DBVisualizer( JDBC to connect to the cluster). 
```sql
...
```
The implementation of _runJob_ function of SnappySQLJob can use SnappyContext to interact with SnappyData store to process and store tables. The implementation of runJob of SnappyStreamingJob can use SnappyStreamingContext to create streams and manage the streaming context.

In this article, we showcase mostly the same set of SnappyData features via Spark API and using SQL. 
...
snappyContext.createTable("AIRLINEREF", "row", schema, Map())
```
#### Action - Create column table, row table and load data
...
#### Action - Run OLAP and OLTP queries
...
```
The following scala code also does the same thing.
 
```
snappyContext.createTable(sampleTable, 
...
Same thing can be achieved using scala APIs.
...
#### Action - Create, Load and Query Sample Table
