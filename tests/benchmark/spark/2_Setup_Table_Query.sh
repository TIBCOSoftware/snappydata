...
#Execute Spark App
sh $SnappyData/bin/spark-submit --master spark://$master:7077 $sparkProperties --class io.snappydata.benchmark.snappy.tpch.SparkApp $TPCHJar $dataDir $NumberOfLoadStages $Parquet $queries $sparkSqlProperties $IsDynamic $ResultCollection $WarmupRuns $AverageRuns 1 $rePartition $IsSupplierColumnTable $buckets_Supplier $buckets_Order_Lineitem $buckets_Cust_Part_PartSupp
