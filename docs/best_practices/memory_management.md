...
### Heap Execution Pool: During query execution or while running a Spark job, all temporary object allocations come out of this pool. For instance, queries like HashJoin and aggregate queries creates expensive in memory maps. This pool is used to allocate such memory.

At the start, each of the two pools is assigned a portion of the available memory. The is driven by `spark.memory.storageFraction` property (default 50%). So by default storage pool and execution pool is assigned 50% of total workable memory.
While you can change `spark.memory.storageFraction` at start up, it is recommended that you do not change this setting unless you are absolutely sure. 

SnappyData allows each pool to "balloon" into the other if capacity is available subject to following rules:
