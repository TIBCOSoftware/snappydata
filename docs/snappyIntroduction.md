...
**2.** high SQL standards conformance, e.g., allowing tables alterations, constraints, indexes  
**3.** support declarative stream processing in SQL. (See section 4)

**(c) Optimizing Spark application execution times:** Our goal is to eliminate the need for yet another external store (e.g., a KV store) for Spark applications. With a deeply integrated store, SnappyData improves overall performance by minimizing network traffic and serialization costs. In addition, by promoting colocated schema designs (tables and streams) where related data is colocated in the same process space, SnappyData eliminates the need for shuffling altogether in several scenarios. We describe and evaluate these optimizations in sections 7 and 8.
