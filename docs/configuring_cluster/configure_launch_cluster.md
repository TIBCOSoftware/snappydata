# Configuring, Launching a SnappyData Clusters
...
## Launch Single node Cluster with Default Configuration
...
If you are launching on a single node, for example, on your laptop or a linux server you have access to, you can do so using this simple command:
...
!note - By default, the locator uses 1527 to listen for client connections and servers running on the same machine use subsequent port numbers. So, 1528 by the single server launched by the above command. But, if the server was launched on a different machine it would listen on 1527 also. 
...
!!!Note
  More examples for loading data from external sources available [here](where is it?)
...
This would load  about 10% of the data from the external data source. Note that the fraction value (0.1, which is 10%) should be reduced when working with large data sets or increased when the data set is small. You must repeat this process for all the tables that you want to manage in memory. 
...
In the following configuration, you are specifying the [number of cores to use on each server](link to this property in ref) as well as enabling the SnappyData Zeppelin interpreter
