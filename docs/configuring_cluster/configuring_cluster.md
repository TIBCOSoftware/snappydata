...
*   [Configuring TIBCO ComputeDB Smart Connector](#configure-smart-connector) 
*   [Logging](#logging)
...
Create the configuration file (**servers**) for data servers in the **<_TIBCO_ComputeDB_home_>/conf** directory. 
...
-   The peer discovery port. TIBCO ComputeDB members connect to the locator for peer-to-peer messaging. The locator port is configurable using the `-peer-discovery-port` option to the snappy server or snappy locator command.
