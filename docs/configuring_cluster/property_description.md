...
|-snappydata.sql.hashJoinSize|The join would be converted into a hash join if the table is of a size that is less than the **hashJoinSize**. The limit specifies an estimate on the input data size (in bytes or k/m/g/t suffixes for a unit). The default value is 100MB.|
