...
|SSL properties|[ssl-enabled](ssl_enabled.md), [ssl-ciphers](ssl_ciphers.md), [ssl-protocols](ssl_protocols.md), [ssl-require-authentication](ssl_require_auth.md). These properties need not be added to  the Lead members in case of a client-server connection.|Server</br>Lead</br>Locator</br>|
