...
- Update Intellij to the latest 16.x version, including the latest Scala plugin. Older versions have trouble dealing with scala code particularly some of the code in _spark_. Newer versions have trouble running tests with gradle import since they do not honour the build output directory as set in gradle.
