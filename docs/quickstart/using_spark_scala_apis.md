...
**Create a SnappySession**:</br>
A SnappySession extends the SparkSession so you can mutate data, get much higher performance, etc.
...
**Create a small dataset using Spark's APIs**:</br>

```scala
scala> val ds = Seq((1,"a"), (2, "b"), (3, "c")).toDS()
```

**Define a schema for the table**:</br>
...
**Insert the created DataSet to the column table "colTable"**:</br>
...
```
**Create a row object using Spark's API and insert the Row to the table**:</br>
...
**Create a "row" table with a simple schema [String, Int] and default options**: </br>
For detailed option refer to the [Row and Column Tables](../programming_guide/tables_in_snappydata.md#row-and-column-tables) section.
...
**Insert the created DataSet to the row table "rowTable"**:</br>
...
**Change some data in a row table**:</br>
