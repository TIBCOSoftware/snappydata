...
In this topic, we explain how SnappyData's Smart Connector application uses the JDBC streaming connector to pull changed data from Microsoft SQL Server and ingest it into SnappyData tables.

!!!Note:
	Writing data into two different tables from a single table is currently not supported as the schema for incoming DataFrame cannot be changed. Currently, one to one correspondence between source table and destination table is supported.
...
Ensure that change data capture is enabled for the database and table level. Refer to your database documentation for more information on enabling CDC for your database and tables.

### Step 2: Enable Streaming for the Source Tables
...
- **connectionOptions**: A map of key values containing different parameters to access the source database.
    ...
    |`poolImpl`|Connection pool implementation(default hikari) Only tomcat & hikari are supported|
    |`maximumPoolSize`|Connection pool size ( default 10)|
    |`minimumIdle`|Minimum number of idle connection( default 5)|

### Step 3: Operating on the reader with normal Structured streaming APIs
...
```
### Step 4: Writing into SnappyData tables
...
The **sink** option is mandatory for SnappyStore sink. This option is required to give the user control of the obtained dataframe. You can however provide any custom option when writing the streaming data to the tables.

| Key | Value |
|--------|--------|
|sink|A user defined callback class which will get a dataframe in each batch. The class must implement org.apache.spark.sql.streaming.jdbc.SnappyStreamSink interface.|
  ...
  public void process(SnappySession snappySession, Properties sinkProps,
    ...
    Dataset<Row> snappyCustomerDelete = df
      ...
      if(batchId == 0){ // Batch ID will be always 0 when stream app starts
        // Incase of 0th batchId always do a putInto , as we don't know if its restaring from a failure.
...
**Offset Management**: One table per application is maintained for offset management (Offset table name for application = AppName_SNAPPY_JDBC_STREAM_CONTEXT). </br>
...
If connector application crashes, it refers to this table on restart to query further.
