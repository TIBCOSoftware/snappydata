...
- [Example: Multiple Servers Using Default Multicast Port for Peer Discovery](#default-port)

- [Example: Servers and Client Using Locator](#servers)

- [Example: Multiple Locators for High Availability](#locators)

- [Example: Servers and Accessors with a Locator](#accessors)

- [Example: Servers and Locator Using BUILTIN Authentication](#authentication)

- [Example: Servers and Accessors with Server Groups and Locator](#server-groups)
...
``` pre
snappy server status [ -dir=<workingdir> ]
```

To stop a running server:

``` pre
snappy server stop [ -dir=<workingdir> ]
```
...
``` pre
snappy server wait [-J<vmarg>]* [-dir=<workingdir>]
```
...
The table describes the options of the snappydata server command. Default values are used if you do not specify an option.
...
As the output above indicates, a *network server* is also started by default that binds to localhost on port 1527. This service allows thin clients to connect to the server and execute SQL commands using the [DRDA protocol](http://en.wikipedia.org/wiki/DRDA.md).
...
``` pre
 snappy server start -dir=server2
-client-port=1528
...
snappy server start -dir=dir2 -client-port=1528
–- start another server talking to the first two with dir3 as working
–- directory; network server is disabled explicitly
snappy server start -dir=dir3 -run-netserver=false
–- check from the SnappyData command shell
snappy
...
–- stop everything
snappy> quit;
snappy server stop -dir=dir3
snappy server stop -dir=dir2
snappy server stop
```
<a id="servers"></a>
...
snappy locator start -peer-discovery-port=3241
–- start three servers as before using different client ports 
-- and using the above started locator
snappy server start -dir=dir1 -locators=localhost:3241 
     -client-port=1528
snappy server start -dir=dir2 -locators=localhost:3241 
     -client-port=1529
snappy server start -dir=dir3 -locators=localhost:3241 
     -client-port=1530
...
snappy locator start -dir=loc2 -peer-discovery-port=3242 
     -locators=localhost:3241 -client-port=1528
–- start four servers that can talk to both 
-- the above locators
snappy server start -dir=dir1 
     -locators=localhost:3241,localhost:3242 
     -client-port=1529
snappy server start -dir=dir2 
     -locators=localhost:3241,localhost:3242 
     -client-port=1530
snappy server start -dir=dir3 
     -locators=localhost:3241,localhost:3242 
     -client-port=1531
snappy server start -dir=dir4 
     -locators=localhost:3241,localhost:3242 
     -client-port=1532
...
–- stop everything
snappy> quit;
snappy server stop -dir=dir5
snappy server stop -dir=dir4
snappy server stop -dir=dir3
snappy server stop -dir=dir2
snappy server stop -dir=dir1
snappy locator stop -dir=loc2
```
<a id="accessors"></a>
...
snappy server start -dir=dir1 -locators=localhost:3241 -client-port=1528
snappy server start -dir=dir2 -locators=localhost:3241 -client-port=1529
snappy server start -dir=dir3 -locators=localhost:3241 -client-port=1530
–- start a couple of peers that will not host data
snappy server start -dir=dir4 -locators=localhost:3241 -client-port=1531 -host-data=false
snappy server start -dir=dir5 -locators=localhost:3241 -client-port=1532 -host-data=false
–- check from the RowStore command shell
–- connect using the locator's client-port (default 1527) 
...
snappy locator start -peer-discovery-port=3241 -auth-provider=BUILTIN  -snappydata.user.gem1=gem1 -user=gem1 -password=gem1
–- start three servers using different client 
-- ports and using the above started locator
snappy server start -dir=dir1 -locators=localhost:3241 -client-port=1528 -auth-provider=BUILTIN  -snappydata.user.gem1=gem1 -user=gem1 -password=gem1
snappy server start -dir=dir2 -locators=localhost:3241 -client-port=1529 -auth-provider=BUILTIN  -snappydata.user.gem1=gem1 -user=gem1 -password=gem1
snappy server start -dir=dir3 -locators=localhost:3241 -client-port=1530 -auth-provider=BUILTIN  -snappydata.user.gem1=gem1 -user=gem1 -password=gem1
–- check from the RowStore command shell
...
snappy server start -dir=dir1 -locators=localhost:3241 -client-port=1528 -server-groups=ordersdb
snappy server start -dir=dir2 -locators=localhost:3241 -client-port=1529 -server-groups=customers
snappy server start -dir=dir3 -locators=localhost:3241 -client-port=1530 -server-groups=ordersdb,customers
...
snappy server start -dir=dir4 -locators=localhost:3241 -client-port=1531 -host-data=false -server-groups=ordersdb,customers
snappy server start -dir=dir5 -locators=localhost:3241 -client-port=1532 -host-data=false -server-groups=ordersdb,customers
