...
object JdbcExtendedUtils {
  ...
  def readSplitProperty(propertyName: String,
    ...
    val optionsClass = options.getClass.getName
    val params =
      if (optionsClass.contains("CaseInsensitiveMap") ||
          optionsClass.contains("CaseInsensitiveMutableHashMap")) {
        options
      } else {
        new CaseInsensitiveMap(options.asJava).asInstanceOf[java.util.Map[String, String]].asScala
      }
