...
//    TPCHRowPartitionedTable.createPopulatePartSuppTable(usingOptionString, props, snc, path, isSnappy, buckets)
    ...
    if (useIndex) {
      TPCHColumnPartitionedTable.createAndPopulateOrder_CustTable(props, snc, path, isSnappy, buckets)
      TPCHColumnPartitionedTable.createAndPopulateLineItem_partTable(props, snc, path, isSnappy, buckets)
    }
    if (queryPlan) {
