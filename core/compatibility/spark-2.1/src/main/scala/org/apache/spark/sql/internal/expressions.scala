...
case class ApproxColumnExtractor21(child: Expression, name: String,
...
case class TaggedAttribute21(tag: Tag, name: String, dataType: DataType, nullable: Boolean,
  ...
  override def withQualifier(newQualifier: Option[String]): TaggedAttribute = {
    if (newQualifier == qualifier) {
      ...
      TaggedAttribute21(tag, name, dataType, nullable, metadata)(exprId, newQualifier)
    }
  }
}

case class TaggedAlias21(tag: TransformableTag, child: Expression, name: String)(
    override val exprId: ExprId, override val qualifier: Option[String]) extends TaggedAlias

case class ClosedFormColumnExtractor21(child: Expression, name: String, confidence: Double,
