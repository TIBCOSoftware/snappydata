...
import org.apache.spark.sql.execution.streaming.{Sink, SnappyStreamExecution, StreamingQueryListenerBus, StreamingQueryWrapper}
...
class SnappyStreamingQueryManager(snappySession: SnappySession)
  ...
  override private[sql] val stateStoreCoordinator =
    StateStoreCoordinatorRef.forDriver(snappySession.sparkContext.env)
  private val listenerBus = new StreamingQueryListenerBus(snappySession.sparkContext.listenerBus)
   ...
   * @since 2.0.0
   */
  override def active: Array[StreamingQuery] = activeQueriesLock.synchronized {
   ...
   * @since 2.1.0
   */
  override def get(id: UUID): StreamingQuery = activeQueriesLock.synchronized {
   ...
   * @since 2.1.0
   */
  override def get(id: String): StreamingQuery = get(UUID.fromString(id))
  ...
  override def awaitAnyTermination(): Unit = {
   ...
   */
  @throws[StreamingQueryException]
  override def awaitAnyTermination(timeoutMs: Long): Boolean = {
   ...
   * @since 2.0.0
   */
  override def resetTerminated(): Unit = {
    awaitTerminationLock.synchronized {
      lastTerminatedQuery = null
    }
  }
   ...
   * @since 2.0.0
   */
  override def addListener(listener: StreamingQueryListener): Unit = {
   ...
   * @since 2.0.0
   */
  override def removeListener(listener: StreamingQueryListener): Unit = {
    listenerBus.removeListener(listener)
  }

  /** Post a listener event */
  override private[sql] def postListenerEvent(event: StreamingQueryListener.Event): Unit = {
  ...
  private def createQuery(
    ...
    if (snappySession.sessionState.conf.isUnsupportedOperationCheckEnabled) {
      UnsupportedOperationChecker.checkForStreaming(analyzedPlan, outputMode)
    }

    if (snappySession.sessionState.conf.adaptiveExecutionEnabled) {
      throw new AnalysisException(
        s"${SQLConf.ADAPTIVE_EXECUTION_ENABLED.key} " +
            "is not supported in streaming DataFrames/Datasets")
    }

    new StreamingQueryWrapper(new SnappyStreamExecution(
      snappySession,
  ...
  override private[sql] def notifyQueryTermination(terminatedQuery: StreamingQuery): Unit = {
    activeQueriesLock.synchronized {
