...
final class DefaultSource extends ExternalSchemaRelationProvider with SchemaRelationProvider
  ...
  private def getSchema(session: SnappySession,
    ...
    case Some(s) =>
      ...
      Some(StructType(parser.parseSQLOnly(s, parser.tableSchema.run())))
  }
  ...
  override def createRelation(sqlContext: SQLContext, mode: SaveMode,
    ...
    try {
      ...
      session.sessionCatalog.externalCatalog.createTableForRegion(relation.resolvedName,
        relation.provider, relation.schema, relation.origOptions)
  ...
  private[sql] def createRelation(session: SnappySession, mode: SaveMode,
    ...
    val partitioningColumns = StoreUtils.getAndSetPartitioningAndKeyColumns(session, parameters)
