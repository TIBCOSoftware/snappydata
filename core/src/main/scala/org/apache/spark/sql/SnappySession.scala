...
import org.apache.spark.sql.catalyst.expressions.{Alias, Ascending, AttributeReference, Descending, Exists, ExprId, Expression, GenericRow, ListQuery, ParamLiteral, ScalarSubquery, SortDirection, TokenLiteral}
  ...
  */
  ...
  private[sql] def getParameterValue(questionMarkCounter: Int, pvs: Any): (Any, DataType) = {
    ...
    protected[sql] def queryPreparations(topLevel: Boolean): Seq[Rule[SparkPlan]] = Seq(
      ...
      InsertCachedPlanFallback(self, topLevel),
...
object SnappySession extends Logging {
  ...
  private def planExecution(qe: QueryExecution, session: SnappySession, sqlText: String,
    ...
    try {
      ...
      session.sparkContext.listenerBus.post(SparkListenerSQLExecutionStart(
  ...
  def sqlPlan(session: SnappySession, sqlText: String): CachedDataFrame = {
    val parser = session.sessionState.sqlParser.asInstanceOf[SnappySqlParser]
    val parsed = parser.parsePlan(sqlText)
    val planCaching = session.planCaching
    val plan = if (planCaching) session.preCacheRules.execute(parsed) else parsed
