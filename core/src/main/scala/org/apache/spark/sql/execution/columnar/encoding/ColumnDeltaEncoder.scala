...
final class ColumnDeltaEncoder(val hierarchyDepth: Int) extends ColumnEncoder {
  ...
  override def initialize(dataType: DataType, nullable: Boolean, initSize: Int,
    ...
    positionsArray = new Array[Long](initSize)
  ...
  def setUpdatePosition(position: Long): Unit = {
    // sorted on LSB so position goes in LSB
    positionIndex += 1
    if (positionIndex == maxSize) {
      val newPositionsArray = new Array[Long](maxSize << 1)
  ...
  private def consumeDecoder(decoder: ColumnDecoder, decoderNullOrdinal: Int,
      decoderBytes: AnyRef, writer: DeltaWriter, encoderCursor: Long,
      encoderOrdinal: Int, doWrite: Boolean = true): Long = {
    if (decoderNullOrdinal >= 0) {
      // nulls are always written as per relative position in decoder
      if (decoder.isNullAt(decoderBytes, decoderNullOrdinal)) {
    ...
    writer.readAndEncode(decoder, decoderBytes, realEncoder, encoderCursor,
      encoderOrdinal, doWrite)
  }

  private def writeHeader(columnBytes: AnyRef, cursor: Long, numNullWords: Int,
      numBaseRows: Int, positions: Array[Long], numDeltas: Int): Long = {
    ...
    var i = 0
    while (i < numDeltas) {
      ColumnEncoding.writeLong(columnBytes, deltaCursor, positions(i))
      deltaCursor += 8
      i += 1
    }
  ...
  private def initializeDecoder(columnBytes: AnyRef, cursor: Long): Long = {
    ...
    val positionEndCursor = tmpPositionCursor + (tmpNumPositions << 3)
  ...
  def merge(newValue: ByteBuffer, existingValue: ByteBuffer,
    ...
    var position1 = ColumnEncoding.readLong(columnBytes1, positionCursor1)
    positionCursor1 += 8
    var position2 = 0L
    if (existingIsDelta) {
      position2 = ColumnEncoding.readLong(columnBytes2, positionCursor2)
      positionCursor2 += 8
    }
    ...
    def isEqualOrGreater(p1: Long, p2: Long) : (Boolean, Boolean) = if (noDuplicateElimination) {
    ...
    while (doProcess) {
      ...
      if (if (noDuplicateElimination) !isGreater && !areEqual else !isGreater) {
        ...
        cursor = consumeDecoder(decoder1, if (nullable1) relativePosition1 else -1,
          columnBytes1, writer, cursor, encoderOrdinal)
        relativePosition1 += 1
        if (relativePosition1 < numPositions1) {
          position1 = ColumnEncoding.readLong(columnBytes1, positionCursor1)
          positionCursor1 += 8
        } else {
          doProcess = false
        }
    ...
    while (relativePosition1 < numPositions1) {
      ...
      // set next update position to be from first
      if (existingIsDelta) {
        positionsArray(encoderOrdinal) = ColumnEncoding.readLong(columnBytes1, positionCursor1)
        positionCursor1 += 8
      }
      cursor = consumeDecoder(decoder1, if (nullable1) relativePosition1 else -1,
        columnBytes1, writer, cursor, encoderOrdinal)
    ...
    while (relativePosition2 < numPositions2) {
      ...
      // set next update position to be from second
      if (existingIsDelta) {
        positionsArray(encoderOrdinal) = ColumnEncoding.readLong(columnBytes2, positionCursor2)
        positionCursor2 += 8
      }
      cursor = consumeDecoder(decoder2, if (nullable2) relativePosition2 else -1,
        columnBytes2, writer, cursor, encoderOrdinal)
    ...
    val positionsSize = if (existingIsDelta) {
      4 /* numBaseRows */ + 4 /* numPositions */ + (numElements << 3)
    } else 0
  ...
  def finish(encoderCursor: Long, numBaseRows: Int): ByteBuffer = {
    ...
    val buffer = allocator.allocateForStorage(ColumnEncoding.checkBufferSize((((8L +
        ...
        4 /* numBaseRows */ + 4 /* numPositions */ + (numDeltas << 3) + 7) >> 3) << 3) +
...
abstract class DeltaWriter {
  ...
  def readAndEncode(srcDecoder: ColumnDecoder, srcColumnBytes: AnyRef,
      destEncoder: ColumnEncoder, destCursor: Long, encoderOrdinal: Int,
      ...
      override def load(dataType: DataType): DeltaWriterFactory = {
        ...
        val expression = if (complexType.isEmpty) {
          s"""
             ...
             |      ColumnEncoder destEncoder, long destCursor, int encoderOrdinal,
             |      boolean doWrite) {
