...
class OpLogRdd(
  ...
  def getProjectColumnId(tableName: String, columnName: String): Int = {
    val fqtn = tableName.replace(".", "_").toUpperCase()
    val maxVersion = versionMap.getOrElse(fqtn, null)
    logInfo(s"getProjectColumnId: fqtn $fqtn  maxN: $maxVersion\ntableSchemas $tableSchemas")
  ...
  def getSchemaColumnId(tableName: String, colName: String, version: Int): Int = {
    val fqtn = tableName.replace(".", "_").toUpperCase()
    var index = -1
    logInfo(s"getProjectColumnId: fqtn $fqtn  maxN: $version\ntableSchemas $tableSchemas")
  ...
  def getRowFormatter(versionNum: Int, schemaStruct: StructType): RowFormatter = {
    ...
    logInfo(s"getRowFormatter: cdl : $cdl, schemaname: $schemaName, tableName: $tableName, versionNum: $versionNum")
  ...
  def getPlaceHolderDiskRegion(diskStr: DiskStoreImpl, regPath: String): PlaceHolderDiskRegion = {
    ...
    for ((_, adr) <- diskStr.getAllDiskRegions) {
      ...
      if (regUnescPath.contains(wrongTablePattern)) {
        ...
        logInfo("correct pattern = " + regUnescPath)
      }
      ...
      if (regUnescPath.equals(regionPath)) {
        ...
        logInfo(s"regunesc != regionpath. ${regUnescPath} != ${regionPath}")
      }
    }
    assert(phdr != null, s"PlaceHolderDiskRegion not found for regionPath=${regPath}")
    logInfo("phdr is null? " + (phdr == null))
    phdr
  }
  ...
  def iterateRowData(phdrRow: PlaceHolderDiskRegion): Iterator[Row] = {
    def getFromDVD(schema: StructType, dvd: DataValueDescriptor, i: Integer,
      ...
      val field = schema(i)
      logInfo(s"getFromDVD: schema: $schema, dvd: $dvd, i: $i, complexSch: $complexSchema")
      ...
      field.dataType match {
        ...
        case _ => {
          ...
          logInfo(s"getFromDVD - returning - ${res}")
          res
        }
      }
    }
    ...
    if (rm != null) {
      logInfo("from iterrowdata. is rm null?" + (rm == null))
      logInfo(s"regionmap entries null? ${rm.regionEntries() == null}")
      if (rm.regionEntries().size() > 0) {
        ...
        regMapItr.map { regEntry =>
          logInfo(s"oplogRdd: testing: schema name : $dbTableName  & table name : $tblName")
          logInfo(s"oplogRdd : tableSchemas:" + s" ${tableSchemas} ")
          ...
          DiskEntry.Helper.readValueFromDisk(
            ...
            case valueArr: Array[Byte] => {
              ...
              // TODO handle null
              val tblName = versionNum + "#" + dbTableName.toLowerCase().replace(".", "_")
              var schemaOfVersion = tableSchemas
                  .getOrElse(tblName.toUpperCase(), new StructType())
              ...
              logInfo(s"oplogrdd:iterateRowData: schStruct: $schemaOfVersion")
              ...
              logInfo(s"rowasseq[] as string: ${row}")
            ...
            case valueArr: Array[Array[Byte]] => {
              ...
              logInfo(s"looking for ${versionNum}#${dbTableName.toLowerCase().replace(".", "_")} and tableSchemas size = ${tableSchemas.size}")
              val tblName = versionNum + "#" + dbTableName.toLowerCase().replace(".", "_")
              var schStruct = tableSchemas
                  .getOrElse(tblName.toUpperCase(), null)
              ...
              logInfo(s"oplogRdd - check valueArr[][]'s first byte" +
            ...
            val decodersAndValues = sch.map { field =>
              ...
              if (hasTombstone) null
                ...
                val valueArray = if (valueBuffer == null || valueBuffer.isDirect) {
                  logWarning(s"valueBuffers is direct : ${valueBuffer.isDirect}")
                  null
                } else {
                  valueBuffer.array()
                }
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[Row] = {
    ...
    logInfo(s"rowregpath=${rowRegPath} colregpath=${colRegPath}")
    for (d <- diskStrs.asScala) {
      ...
      for ((_, adr) <- dskRegMap.asScala) {
        ...
        if (adrUnescapePath.contains(wrongTablePattern)) {
          ...
          logInfo("correct pattern = " + adrUnescapePath)
        }
        logInfo(s"adrpath=${adrPath} adrunescapepath=${adrUnescapePath}")
    ...
    assert(diskStrRow != null, s"row disk store is null. diskStrRow=" + diskStrRow)
    ...
    if (decoder.isNullAt(value, rowNum)) null
      ...
      case StringType => {
        try {
          ...
          case e: Throwable =>
            logInfo(s"OplogRDD: row num = $rowNum. stacktrace: ${e.getStackTraceString}")
        }
      }
      case DateType =>
  ...
  override protected def getPartitions: Array[Partition] = {
    logInfo(s"getPartition: projection $projection")
    println(s"oplogrdd: getPartitions:\ndbTableName:" +
        s" $dbTableName\ntblName: $tblName\nsch: $sch\n provider: $provider\n")
    logInfo(s"oplogrdd: getPartitions:\ndbTableName:" +
        s" $dbTableName\ntblName: $tblName\nsch: $sch\n provider: $provider\n")
    ...
    logInfo(s"getpartition: numbuckets $numBuckets isReplicated $isReplicated")
    val partition = (0 until numBuckets).map { p =>
      new Partition {
        override def index: Int = p
      }
    }.toArray[Partition]
    logInfo(s"oplogrdd:getPartition: no. of partitions ${partition.length}")
    partition.foreach(e => logInfo(s"oplogrdd:getpartition: ${e.index}"))
    partition
  }
  ...
  override def getPreferredLocations(split: Partition): Seq[String] = {
    logInfo(s"oplogrdd getpreferredlocation: split - $split    dbTableName: $dbTableName")
    val host = RecoveryService.getExecutorHost(dbTableName, split.index)
    logInfo(s"preferred host location for split: ${split.index} is ${host}")
    host
  }
