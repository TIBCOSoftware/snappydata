...
import org.apache.spark.sql.internal.{DeltaInsertIntoColumnTable, PutIntoColumnTable}
...
object StoreStrategy extends Strategy {
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    ...
    case DeltaInsertIntoColumnTable(LogicalRelation(p: DeltaInsertRelation, _, _), left, right) =>
      ExecutePlan(p.getDeltaInsertPlan(planLater(left), planLater(right))) :: Nil
