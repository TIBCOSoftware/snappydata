...
class QueryProcessingSuite extends FunSuite with BeforeAndAfter with Logging {
  ...
  test("Run a sum query on sample table if pre query triage qualifies the query to run on sample table") {
    ...
    snContext.dropTempTable("people_sampled")
    println("Success...Test1")
  }

  test("Run a avg query on sample table if pre query triage qualifies the query to run on sample table") {
    ...
    // Run query on sample table
    val sampled_result = snContext.sql("SELECT AVG(age) FROM people where age > 35 ERRORPERCENT 8.5" )
    expected = "[41.0]"
    sampled_result.collect.foreach(verifyResult)
    println("Success...Test2")
  }

  test("Run query on appropriate sample table if multiple sample tables for a sample table available") {
