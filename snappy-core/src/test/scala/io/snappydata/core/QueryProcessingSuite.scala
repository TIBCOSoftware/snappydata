...
class QueryProcessingSuite extends FunSuite with BeforeAndAfter with Logging {
  ...
  test("Run a query on sample table if pre query triage qualifies the query to run on sample table") {
   ...
   // ))
    ...
    val rowRDD = sc.parallelize(people, people.length).map(s =>
      new Data(s(0).asInstanceOf[String], s(1).asInstanceOf[Int]))
...
case class Data(name: String, age: Int) extends Serializable