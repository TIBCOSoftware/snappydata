...
private[sql] final class CachedBatchHolder[T](getColumnBuilders: => Array[ColumnBuilder],
   ...
   */
  private def appendRow_(newBuilders: Boolean, row: InternalRow, forceCreate: Boolean): Unit = {
  ...
  def appendRow(u: Unit, row: InternalRow): Unit =
    appendRow_(newBuilders = true, row, false)
  ...
  def forceEndOfBatch(): T = {
    if (rowCount > 0) {
      ...
      appendRow_(newBuilders = false, expressions.EmptyRow, true)
    }
    result
  }
}
