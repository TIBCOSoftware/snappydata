...
class SnappyJoinSuite extends FunSuite with Logging  with BeforeAndAfterAll{
  ...
  test("Replicated table join with PR Table") {
    ...
    snc.sql("CREATE TABLE RR_TABLE2(OrderRef INT NOT NULL,description String) USING row options()")
    ...
    val countDf = snc.sql("select * from PR_TABLE P JOIN RR_TABLE R ON P.ORDERREF = R.REF")
    val t1 = System.currentTimeMillis()
    assert(countDf.count() === 1000) // Make sure aggregation is working with local join
    val t2 = System.currentTimeMillis()
    println("Time taken = "+ (t2-t1))

    val projectDF = snc.sql("select ORDERID, P.DESCRIPTION , R.DESCRIPTION from PR_TABLE P JOIN RR_TABLE R ON P.ORDERREF = R.REF")
    assert(projectDF.columns.length === 3)

    val sumDF = snc.sql("select SUM(ORDERID)from PR_TABLE P JOIN RR_TABLE R ON P.ORDERREF = R.REF")
