...
object TPCH_Snappy {
  ...
  def execute(queryNumber: String, snappyContext: SnappyContext, isResultCollection: Boolean, isSnappy:Boolean, itr : Int): Unit = {
    ...
    if (itr == 1) {
      ps.println("partitions = 100 ")
      snappyContext.sql("set spark.sql.shuffle.partitions=100")
    }
    try {
      println(s"Started executing $queryNumber")
      if (isResultCollection) {
        val cnts = queryExecution(queryNumber, snappyContext, isSnappy)
  ...
  def queryExecution(queryNumber:String, snappyContext:SnappyContext, isSnappy:Boolean): scala.Array[org.apache.spark.sql.Row] ={
    //val snappyContext  = SnappyContext.getOrCreate(sc)
